    ...
    /* USER CODE BEGIN 3 */
		...
		/* 1. Detección de flanco (Usa el driver genérico del botón afuera del switch) */
		        if (BOTON_DetectarFlancoPresionado(INDICE_BTN_USER)) {
		            sentido *= -1; // Inversión de dirección instantánea
		        }

		        /* 2. Máquina de Estados Finitos clásica bloqueante */
		        switch (estado_mef) {

		        case Estado_Encendido:
		            LED_Write(indice_led, true);
		            TIME_DelayMS(tiempo_alternancia); // Reemplaza a HAL_Delay
		        ...
		        case Estado_Apagado:
		            ...
		            // Lógica circular adaptada a las funciones de tu driver
